...
Selected symbols from the language that are used in this architecture are described in an element table. Obects are represented as "architectural building blocks" (ABBs) and Solution Building Blocks (SBBs). The ABBs have a lighter color than the SBBs.
